...
## [V1.0] - 2026-08-20 (深色 UI 大版本改版 · 测试版)
### Changed
- **全新深色 UI 主题（纯视觉，功能零改动）**: 依据产品方提供的界面设计细节，整套界面由浅色改为现代深色风格。
  - 强调色 `#0A84FF` → `#2997FF`；工具栏 / 颜色面板底色改为石墨黑 `rgba(24,24,27,0.96)`；调色板、选中高亮态同步换蓝。
  - 新增 `tray_menu_css`，托盘右键菜单深色化（不再原生浅色）。
  - 重画 5 个工具栏 SVG 图标：随意画线（平滑曲线）、文字（闭合 T 形）、取消（居中圆头红叉）、确认（蓝对勾）、撤销（↶ 半圆回旋箭头）。
  - 全屏蒙版保持黑 39% `rgba(0,0,0,100)`；选区描边保持主题蓝 `#2997FF`。
  - `logo.ico`（窗口 / 托盘 / exe 三处共用）换新：深色圆角磁贴 + 蓝色光标箭头（多尺寸 16/24/32/48/64/128/256 px）。
- **版本线重置**: 从 V107 直接升到 **V1.0**，作为深色 UI 时代的正式起点（本次为测试版 / Beta，功能与 V107 完全一致）。
- 版本号三处同步更新: `main.pyw` docstring + 启动日志 `=== Mouse4 V1.0 (测试版) Started ===` + 本 CHANGELOG；并新增存档快照 `archive/main.V1.0.pyw`。
### Notes
- 本次为**测试版(Beta)**: 仅外观改版，截图/保存/快捷键/托盘逻辑一字未动，`py_compile` 已通过。
- 同步更新 `RELEASE_NOTE.md`、`README.md` 的深色 UI 章节。
