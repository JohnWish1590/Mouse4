...
def run_paste_mode_safe(args):
    try:
        ...
        if len(args) > 2 and '--paste' in args:
            try:
                ...
                if idx + 1 < len(args): target_folder = " ".join(args[idx+1:]).strip('"').strip()
            except: pass
    ...
    except Exception as e: config_mgr.log(f"[PasteMode] Error: {e}")
    sys.exit(0)
...
class SnippingToolBar(QWidget):
    def __init__(self, parent=None):
        ...
        for b in [self.btn_rect, self.btn_ellipse, self.btn_arrow, self.btn_pen, self.btn_text, self.btn_undo, self.btn_cancel, self.btn_ok]: t_layout.addWidget(b)
...
class SnippingWindow(QWidget):
    def __init__(self, screen_info):
        ...
        self.active_input = None; self._last_click_time = 0; self._double_click_threshold = 400
        
        self.toolbar = SnippingToolBar(self); self.toolbar.hide(); self.setup_ui()
    ...
    def grab_current_screen(self):
        try:
            with mss.mss() as sct:
                geo = self.geometry(); cx, cy = geo.x() + geo.width()//2, geo.y() + geo.height()//2
                monitor = sct.monitors[1]
                for m in sct.monitors[1:]:
                    if m['left'] <= cx < m['left']+m['width'] and m['top'] <= cy < m['top']+m['height']:
                        monitor = m; break
                img = sct.grab(monitor); self.scale_factor = img.width / max(1, geo.width())
    ...
    def set_draw_mode(self, mode):
        ...
        self.draw_mode = mode; self.toolbar.colors_widget.setVisible(mode is not None)
    ...
    def paintEvent(self, event):
        ...
        if self.full_screenshot: painter.drawPixmap(self.rect(), self.full_screenshot)
        painter.setBrush(QColor(0,0,0,100)); painter.setPen(Qt.PenStyle.NoPen)
        if not self.has_selected and not self.is_selecting: painter.drawRect(self.rect())
        else:
            painter.drawRect(self.rect()); rect = QRect(self.begin, self.end).normalized()
            if not rect.isEmpty():
                ...
                for item in self.drawings: painter.setPen(QPen(item['color'], 2)); self.draw_item(painter, item)
                if self.current_drawing: painter.setPen(QPen(self.current_color, 2)); self.draw_item(painter, self.current_drawing)
                txt = f"{sw} x {sh}"; painter.setPen(Qt.PenStyle.NoPen); painter.setBrush(QColor('black'))
                painter.drawRect(rect.x(), rect.y()-25, len(txt)*9, 20)
                painter.setPen(QColor('white')); painter.setFont(QFont("Arial", 10, QFont.Weight.Bold)); painter.drawText(rect.x()+5, rect.y()-10, txt)
    ...
    def draw_item(self, p, item):
        ...
        if t == 'rect': p.drawRect(item['rect'])
            ...
            p.setPen(QPen(item['color'])); p.drawText(item['point'], item['text'])
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton and self.has_selected:
            ...
            if curr - self._last_click_time < self._double_click_threshold: self.finish_capture(); return
    ...
    def mouseMoveEvent(self, event):
        if self.current_drawing:
            self.current_drawing['end'] = event.pos(); self.current_drawing['rect'] = QRect(self.current_drawing['start'], event.pos()).normalized()
    ...
    def mouseReleaseEvent(self, event):
        if self.current_drawing: self.drawings.append(self.current_drawing); self.current_drawing = None; self.update()
        elif self.is_selecting:
            self.is_selecting = False; self.has_selected = True; self.end = event.pos(); rect = QRect(self.begin, self.end).normalized()
    ...
    def _do_save_sync(self, rect):
        try:
            ...
            if self.drawings:
                canvas = QPixmap(cropped_raw.size()); canvas.fill(Qt.GlobalColor.transparent); p = QPainter(canvas)
                ...
                for item in self.drawings: p.setPen(QPen(item['color'], 2)); self.draw_item(p, item)
            ...
            buf_png = BytesIO(q_buffer.data().data())
            ...
            config_mgr.log(f"[Clipboard] Saved {sw}x{sh} (via QBuffer & PIL)")
    ...
    def close_all(self): close_all_windows()
...
class RegistryManager:
    ...
    def install(self):
        try:
            exe_path = sys.executable; command_str = f'"{exe_path}" --paste "%V"'
            key = winreg.CreateKey(winreg.HKEY_CURRENT_USER, self.key_path)
            winreg.SetValue(key, "", winreg.REG_SZ, config.context_menu_text)
            winreg.SetValueEx(key, "Icon", 0, winreg.REG_SZ, exe_path); winreg.CloseKey(key)
    ...
    def uninstall(self):
        try:
            ...
            winreg.DeleteKey(winreg.HKEY_CURRENT_USER, self.key_path); return True, "已移除。"
...
class SuccessToast(QWidget):
    def __init__(self, text, parent=None):
        ...
        self.anim = QPropertyAnimation(self, b"windowOpacity"); self.anim.setDuration(1500); self.anim.finished.connect(self.close)
    def show_anim(self, x, y): self.move(x, y-30); self.show(); self.anim.setStartValue(1); self.anim.setEndValue(0); self.anim.start()
...
def setup_tray(app):
    ...
    tray_icon = QSystemTrayIcon(icon, app); m = QMenu()
    m.addAction("立即截图 (Ctrl+1)", comm.trigger_screenshot.emit)
    m.addAction("重启程序 (修复)", restart_program); m.addSeparator()
    ...
    m.addSeparator(); m.addAction("Github", lambda: webbrowser.open(config.github_url)); m.addAction("退出", app.quit)
