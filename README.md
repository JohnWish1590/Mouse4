...
<p align="center">
  <img src="https://img.shields.io/badge/version-V1.0(测试版)-blue.svg" alt="Version V1.0 测试版">
...
## 🌑 本次大版本：深色 UI 全面改版（V1.0）

V1.0 是一次**纯视觉的大版本改版**，**功能代码一行未动**——截图、保存、快捷键、托盘逻辑与 V107 完全一致，只是把整套界面从浅色换成现代深色风格。设计规格如下（依据产品方提供的界面设计细节）：

| 项目 | 旧版 | V1.0 新版 |
|---|---|---|
| 主题强调色 | `#0A84FF` | **`#2997FF`**（更亮的系统蓝） |
| 工具栏 / 颜色面板底色 | 浅色 | **石墨黑 `rgba(24,24,27,0.96)`** |
| 调色板 / 选中高亮 | 旧蓝 | 同步换为新蓝 `#2997FF` |
| 托盘右键菜单 | 原生浅色 | **深色化**（新增 `tray_menu_css`） |
| 全屏蒙版 | 黑 39% `rgba(0,0,0,100)` | 保持黑 39%（"保持现状"） |
| 选区描边 | 主题蓝 | 主题蓝 `#2997FF`（不变） |
| 程序图标 logo.ico | 旧图标 | **深色圆角磁贴 + 蓝色光标箭头**（7 档尺寸 16/24/32/48/64/128/256 px） |

**重画的 5 个工具栏图标（SVG）**：随意画线（平滑曲线）、文字（闭合 T 形）、取消（居中圆头红叉）、确认（蓝对勾）、**撤销（↶ 半圆回旋箭头）**。

**工具栏 8 图标顺序**（从左到右）：矩形 / 圆形 / 箭头线 / 随意画线 / 文字 / 撤销 / 取消 / 确认。

> 验证要点：托盘图标、exe 图标、窗口图标均为深色圆角方块 + 蓝色光标箭头；截图时全屏铺黑 39% 蒙版 → 顶部浮出深色工具栏（蓝强调色）；右键托盘菜单为深色风格，不再原生浅色。
...
4. 完成后自动：保存 PNG（右上角提示 Saved!）+ 复制到剪贴板。
